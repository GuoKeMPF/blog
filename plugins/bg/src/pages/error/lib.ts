...
// General
var screen: any, game: any;

// Assets
var invaderCanvas: any,
	invaderSize = 20,
...
export class Game {
	...
	onReady: (game: Game) => void;
	constructor({ element, onReady }: GamePropsType) {
		this.level = -1;
		this.lost = false;
		this.invaders = [];
		this.invaderShots = [];
		...
		this.player = new Player({ screen, gameSize, game: this });
	...
	createInvaders = () => {
		var invaders = [];
		var i = blocks.length * this.invaderMultiplier;
		while (i--) {
	...
	update = () => {
		if (!this.lost) {
			// Collision
			this.player.projectile.forEach((projectile: { active: boolean }) => {
	...
	invadersBelow = (invader: { coordinates: { x: number; y: number } }) => {
		return (
...
class Invader implements InvaderType {
	...
	screen: any;
	game: any;
	constructor({ coordinates, speedX, screen, game }) {
		this.active = true;
	...
	update = () => {
		if (Math.random() > invaderAttackRate && !this.game.invadersBelow(this)) {
...
class Player implements PlayerType {
	...
	update = () => {
		...
		if (
			this.keyboarder.keyState === this.keyboarder.KEYS.LEFT &&
			this.coordinates.x > 0
		) {
			this.coordinates.x -= 2;
		} else if (
			this.keyboarder.keyState === this.keyboarder.KEYS.RIGHT &&
			this.coordinates.x < this.gameSize.width - this.size.width
		) {
			this.coordinates.x += 2;
		}

		if (this.keyboarder.keyState === this.keyboarder.KEYS.Space) {
			this.shooterHeat += 1;
...
class Projectile implements ProjectileType {
	...
	screen: any;
	gameSize: any;
	constructor({ coordinates, velocity, screen, gameSize }: ProjectileParams) {
		this.active = true;
		this.coordinates = coordinates;
		this.size = {
			width: 3,
...
// Other functions
// ---------------
const collides = (a: never, b: Invader) => {
	return (
