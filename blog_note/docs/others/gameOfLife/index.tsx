...
const GameOfLife: FC = () => {
  ...
  const [mapConfig, setMapConfig] = useState<MapConfig>(initConfig);

  const [mapSize, setMapSize] = useState<undefined | MapSize>();

  const [initMap, setInitMap] = useState<InitSet>((): InitSet => {
    const map: InitSet = new Map();
    initVal.forEach((i) => {
      const { x, y } = i;
      map.set(`${x}-${y}`, i);
    });
  ...
  const [status, setStatus] = useState<CurrentStatus>('stoped');
  ...
  const start = () => {
    ...
    form
      ...
      .then((values) => {
        ...
        // 需要判断是否有超出地图的值
        const filtered: InitSet = new Map();
