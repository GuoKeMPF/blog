...
import { WebGLRenderer, Color, Scene, PerspectiveCamera, Object3D, HemisphereLight, DirectionalLight, MathUtils } from 'three';
...
export const threejsUtils = {
  ...
  init(options = { threejsOptions: {} }) {
    ...
    const render = (time: number) => {
      ...
      this.elementsOnScreen.forEach(elem => {
        ...
        const wasRendered = fn(renderer, duration, resized);
  ...
  addDiagrams(diagrams) {
    diagrams.forEach(item => {
      ...
      this.addDiagram(element, info)
    });
  },
  addDiagram(elem, info) {
    if (!elem) {
      throw new Error(`no diagram element`);
    }
    if (!info) {
      throw new Error(`no diagram info`);
    }
    this.init();
    ...
    const near = 0.1;
    const far = 50;
    let camera = new PerspectiveCamera(targetFOVDeg, aspect, near, far);
    ...
    promise.then((result) => {
      const info = result instanceof Object3D ? {
        obj3D: result,
      } : result;
      if (info.obj3D) {
        root.add(info.obj3D);
    ...
    const render = (renderer, time) => {
      ...
      const fovDeg = aspect >= 1
        ? targetFOVDeg
        : MathUtils.radToDeg(2 * Math.atan(Math.tan(MathUtils.degToRad(targetFOVDeg) * .5) / aspect));
