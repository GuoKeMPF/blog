...
const Audio: ModelType = {
  ...
  effects: {
    *queryAudios({ payload }, { put, call }) {
      console.log('effect');
      const response: { data: AudioStateType.AudiosType; count: number } =
        yield call(queryAudios, payload);
      if (response) {
        yield put({
          type: 'update',
          payload: {
    ...
    *queryAudioByID({ payload }, { put, call }) {
      const response: { data: AudioStateType.AudiosType } = yield call(
        queryAudioByID,
        payload,
      );
      if (response) {
