...
const gameOfLife = (mapP: { xAxis: any; yAxis: any; width?: number; height?: number; }, listP: { x: number; y: number; }[]) => {
    ...
    const initMap = (list: any[]) => {
        list.forEach((element: { x: any; y: any; }) => {
            const { x, y } = element
            arrs[x][y] = live
        });
    }
    ...
    const nextStep = () => {
        ...
        for (let row = 0; row < mapP.xAxis; row++) {
            ...
            for (let col = 0; col < mapP.yAxis; col++) {
                ...
                let tl = 0, tr = 0, bl = 0, br = 0, t = 0, b = 0
                if (topRow) {
